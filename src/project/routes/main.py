...
@main.get("/investor/<slug>/edit")
@login_required
@check_user_info_complete
@check_verification
def edit_investor_view(slug):
    status_type, msg = None, None
    if query := request.args:
        status_type = query.get("type")
        msg = query.get("msg")
...
@main.post("/investor/<slug>/edit")
@login_required
@check_user_info_complete
@check_verification
def edit_investor(slug):
    investor = Investor.get_by_slug(slug)
    if not investor:
    ...
    if existing_email and existing_email.id != investor.user_id:
        ...
        return redirect(url_for("main.edit_investor_view", slug=slug, _external=True, **status))
