...
- [ ] **Step 1:** `uv add resend`. Add to `Settings`: `resend_api_key: str | None` (alias `_RESEND_API_KEY`), `email_from: str` (alias `_EMAIL_FROM`, default `"Globalify <noreply@mail.globalify.xyz>"`), and an `email_is_configured` property (`bool(resend_api_key)`).
