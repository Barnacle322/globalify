...
def update_company_view(id):
    ...
    if not company:
        ...
        return redirect(url_for("admin.admin_companies_view", _external=True, **status))
...
def update_company(id):
    ...
    if not name:
        status = Status(StatusType.ERROR, "Name cannot be empty!").get_status()
        return redirect(url_for("admin.update_company_view", id=id, _external=True, **status))
...
def create_company():
    ...
    if not name:
        status = Status(StatusType.ERROR, "Name cannot be empty!").get_status()
        return redirect(url_for("admin.create_company_view", _external=True, **status))
...
@admin.post("/company/<int:id>/delete")
@admin_only
